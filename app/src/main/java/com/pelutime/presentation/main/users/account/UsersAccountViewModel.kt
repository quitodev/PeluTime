package com.pelutime.presentation.main.users.account
...
@HiltViewModel
class UsersAccountViewModel @Inject constructor(private val iAccountRepo: IAccountRepo): BaseViewModel() {
    ...
    fun liveDeleteUser(oldPass: String) = liveData(viewModelScope.coroutineContext + Dispatchers.IO) {
        emit(State.Loading())
        try {
            liveGetUserRoom.value!!.mapSuccess { user ->
                iAccountRepo.signIn(user.email, oldPass).mapSuccess { id ->
                    if (id != "Usuario desconectado") {
                        if (user.image != "Vacío") {
                            iAccountRepo.deleteImages(user.id).mapSuccess {
                                iAccountRepo.deleteUser(user.id).mapSuccess {
                                    ...
                                    iAccountRepo.deleteUserAuth().mapSuccess {
                                        emit(State.Success(it))
                                    }
                                }
                            }
